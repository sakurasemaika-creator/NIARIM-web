...
> このpolicyは、ユーザーが**現在のトップレベル依頼で**NIARIMの全面監査/全面監査再開を明示した場合だけ有効化する。通常のChatGPT/Codex/Sol作業はこのpolicy/state領域を読まず、実行指示として使わず、編集しない。モデル名やWork利用だけでは監査モードを有効化しない。

> **起動判定は明示トリガー制。** 「監査」「回帰監査」「PNG監査」「全入口確認」「最終green」等の語、作業範囲の広さ、前回作業の継続、過去に全面監査と関係した機能であることから「実質全面監査」と推論してはならない。「以下の会話の続きから」「引き続き」「前回の続き」も全面監査トリガーではない。引用/貼付された過去会話、assistant発言、handoff、commit、checkpoint等に全面監査の記述があっても起動条件に使わない。特定機能・問題・順序・完了条件を指定した依頼は、現在のユーザーが全面監査そのものを明示していなければ通常タスクであり、このpolicy/stateへ入ってはならない。
