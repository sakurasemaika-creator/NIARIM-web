...
for (const width of widths) {
  ...
  const state = await page.evaluate(() => {
    ...
    const cards = [
      ...
      const cs = getComputedStyle(el),
        ...
        childCs = child ? getComputedStyle(child) : null,
    ...
    const body = getComputedStyle(document.body),
      ...
      footerCs = footer ? getComputedStyle(footer) : null,
      footerBefore = footer ? getComputedStyle(footer, "::before") : null;
    ...
    const finalCta = document.querySelector(".final-cta > .container"),
      finalCtaCs = finalCta ? getComputedStyle(finalCta) : null;
    return {
      rows,
      cards,
      ...
      footer: footer
        ? {
            ...
            dividerHeight: parseFloat(footerBefore?.height || "0"),
            dividerDisplay: footerBefore?.display,
            borderTopWidth: parseFloat(footerCs?.borderTopWidth || "0"),
            cols: footerCols,
          }
        : null,
      finalCta: finalCta
  ...
  if (!state.footer) findings.push({ width, kind: "footer-missing" });
  else {
    ...
    const pseudoDividerVisible =
      state.footer.dividerDisplay !== "none" &&
      state.footer.dividerHeight >= 0.5 &&
      state.footer.dividerBackground !== "none";
    const dividerVisible =
      pseudoDividerVisible || state.footer.borderTopWidth >= 0.5;
    ...
    if (!dividerVisible || !surfacedCols)
      findings.push({
        width,
        kind: "footer-surface-regression",
        footer: state.footer,
      });
  }
  if (!state.finalCta)
  ...
  const featureNav = await page.evaluate(() => {
    ...
    const cs = getComputedStyle(nav),
      after = getComputedStyle(nav, "::after");
    return {
  ...
  if (!featureNav) findings.push({ width, kind: "feature-nav-missing" });
  else {
...
console.log(
  JSON.stringify(
    { auditedWidths: widths, designPolishFindings: findings.length, findings },
    null,
    2,
  ),
);
