...
for (const route of ["/", "/features/"]) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (!(await mock.isVisible())) {
      const classes = (await mock.getAttribute("class")) || "";
      skipped.push({ route, index: i, classes, reason: "not-visible" });
      continue;
    }
...
await fs.writeFile(
  ...
  JSON.stringify({ captures: manifest, skipped }, null, 2),
);
await browser.close();
console.log(
  JSON.stringify(
    {
      ...
      skippedNotVisible: skipped.length,
    },
    null,
    2,
  ),
