...
async function inspectSurface(page, selector) {
  return page.locator(selector).first().evaluate((surface) => {
    ...
    const hasClippingAncestor = (el) => {
      ...
      while (p && p !== surface) {
        const cs = getComputedStyle(p); const ox = cs.overflowX; const oy = cs.overflowY;
    ...
    for (const el of surface.querySelectorAll("*")) {
      ...
      bad.push({ tag: el.tagName, className: typeof el.className === "string" ? el.className : el.getAttribute("class") || "", text: (el.textContent || "").trim().slice(0, 80), left: r.left, right: r.right, top: r.top, bottom: r.bottom });
...
for (const vp of viewports) {
  ...
  for (const language of languages) {
    ...
    if (cardCount !== expectedGalleryCards) failures.push({ kind: "gallery-card-count", viewport: vp.name, language, actual: cardCount, expected: expectedGalleryCards });
    ...
    if (await scroller.count()) {
      await scroller.evaluate((el) => { el.scrollLeft = 0; }); await page.waitForTimeout(80); await shot(scroller, `${prefix}__home__gallery-start.png`);
      await scroller.evaluate((el) => { el.scrollTo({ left: el.scrollWidth, behavior: "instant" }); }); await page.waitForTimeout(180);
      const end = await scroller.evaluate((s) => {
        const last = s.querySelector(":scope > .screenshot-card:last-of-type"); if (!last) return null;
        const sr = s.getBoundingClientRect(); const lr = last.getBoundingClientRect();
    ...
    for (const id of featureIds) {
      ...
      if (!(await section.count())) { failures.push({ kind: "feature-missing", viewport: vp.name, language, id }); continue; }
