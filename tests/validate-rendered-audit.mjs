...
const reportPath = process.env.AUDIT_REPORT || 'artifacts/autonomous-browser-audit/report.json';
const report = JSON.parse(await fs.readFile(reportPath, 'utf8'));

const knownDeferredRule = 'Final download CTA must use brand accent';
const remaining = report.findings.filter(item =>
  !(item.kind === 'design-invariant' && item.detail?.rule === knownDeferredRule)
);
...
for (const vp of viewports) {
  ...
  if (!(await cta.count())) {
    renderedFindings.push({ viewport: vp.name, rule: knownDeferredRule, reason: 'missing' });
    await context.close();
    continue;
  }
  ...
  if (state.actual !== state.expected) {
    renderedFindings.push({ viewport: vp.name, rule: knownDeferredRule, ...state });
  }
...
if (renderedFindings.length) {
  console.error(JSON.stringify({ validated: false, renderedFindings }, null, 2));
  process.exit(1);
}
