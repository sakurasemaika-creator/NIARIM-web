...
const finding = (vp, route, kind, detail) =>
  report.findings.push({ viewport: vp, route, kind, detail });
...
async function inspectViewport(page, vp, route, label) {
  const state = await page.evaluate(async () => {
    ...
    const substantive = visible.filter((el) => {
      if (el.matches("html,body,header,main,footer,section,div,span"))
        return false;
      return (
        (el.textContent || "").trim().length > 0 ||
        el.matches("img,svg,canvas,video,input,textarea,button,a")
      );
    });
    ...
    for (const ref of new Set(useRefs)) {
      ...
      if (!symbolPattern.test(source))
        missingSvgSymbols.push({ ref, reason: "symbol-not-found" });
    }
  ...
  if (state.scrollWidth > state.clientWidth + 2)
    finding(vp, route, "horizontal-overflow", { label, ...state });
  state.broken.forEach((src) =>
    finding(vp, route, "broken-image", { label, src }),
  );
  state.missingSvgSymbols.forEach((detail) =>
    finding(vp, route, "missing-svg-symbol", { label, ...detail }),
  );
  if (state.substantiveCount === 0 && state.docHeight > state.height + 50)
    finding(vp, route, "empty-viewport", { label, ...state });
}
...
async function auditDesignInvariants(page, vp, route) {
  const state = await page.evaluate(() => {
    ...
    const mocks = [
      ...
      return {
        ...
        revealVisible:
          !el.closest(".reveal") ||
          el.closest(".reveal").classList.contains("is-visible"),
      };
    });

    const root = getComputedStyle(document.documentElement);
    const accent = resolveColor(root.getPropertyValue("--color-accent").trim());
    const accentStrongRaw = root
      .getPropertyValue("--color-accent-strong")
      .trim();
    const ctaAccent = resolveColor(
      accentStrongRaw || root.getPropertyValue("--color-accent").trim(),
    );
    const footerBrown = resolveColor(
      root.getPropertyValue("--color-footer-brown").trim() || "#3b2429",
    );
    ...
    const communityCards = document.querySelectorAll(
      ".community-gallery .community-card:not(.is-more-cta)",
    ).length;
    ...
    return {
      ...
      finalPrimaryBg: finalPrimary
        ? getComputedStyle(finalPrimary).backgroundColor
        : null,
      marqueeAnimationName: marquee
        ? getComputedStyle(marquee).animationName
        : null,
  ...
  for (const mock of state.mocks) {
    ...
    if (
      ...
      finding(vp, route, "screen-mock", {
        reason: "hidden-or-reveal",
        ...mock,
      });
    }
  ...
  if (route === "/") {
    if (vp === "sp" && state.heroAlign !== "left") {
      finding(vp, route, "design-invariant", {
        rule: "SP hero must remain left aligned",
        actual: state.heroAlign,
      });
    }
    if (state.primaryBg && state.primaryBg !== state.ctaAccent) {
      finding(vp, route, "design-invariant", {
        rule: "Hero download CTA must use CTA accent token",
        expected: state.ctaAccent,
        actual: state.primaryBg,
      });
    }
    if (state.finalPrimaryBg && state.finalPrimaryBg !== state.ctaAccent) {
      finding(vp, route, "design-invariant", {
        rule: "Final download CTA must use CTA accent token",
        expected: state.ctaAccent,
        actual: state.finalPrimaryBg,
      });
    }
    if (
      state.marqueeAnimationName !== "marquee-scroll" ||
      state.marqueeAnimationDuration === "0s"
    ) {
      finding(vp, route, "design-invariant", {
        rule: "Feature marquee must animate",
        animationName: state.marqueeAnimationName,
        duration: state.marqueeAnimationDuration,
      });
    }
    const nonWhite = state.rowBackgrounds
      .map((color, i) => ({ color, i }))
      .filter((x) => x.color !== "rgb(255, 255, 255)");
    if (nonWhite.length) {
      finding(vp, route, "design-invariant", {
        rule: "Main feature panels must all be white",
        nonWhite,
      });
    }
  }
  ...
  if (state.footerImage && state.footerImage !== "none") {
    finding(vp, route, "design-invariant", {
      rule: "Footer must not use a gradient/image",
      actual: state.footerImage,
    });
  }
  if (state.footerBg && state.footerBg !== state.footerBrown) {
    finding(vp, route, "design-invariant", {
      rule: "Footer must use footer brown",
      expected: state.footerBrown,
      actual: state.footerBg,
    });
  }
  if (route === "/community/" && state.communityCards !== 9) {
    finding(vp, route, "design-invariant", {
      rule: "Community preview must show nine ranked tiles",
      actual: state.communityCards,
    });
  }
}
...
async function exerciseLanguage(page, vp, route, routeName, navToggle) {
  ...
  if (!(await menu.isVisible())) {
    ...
    if (target) {
      ...
      if (changed === original)
        finding(vp, route, "language-menu", "Language did not change");
      ...
      if (await trigger.isVisible()) {
        ...
        const restore = page
          .locator(`.lang-menu button[data-lang-switch="${original}"]`)
          .first();
      ...
      if (restored !== original)
        finding(vp, route, "language-restore", { original, restored });
    }
  }
  if (
    navToggle &&
    (await navToggle.isVisible()) &&
    (await navToggle.getAttribute("aria-expanded")) === "true"
  ) {
...
async function exerciseCommunityTabs(page, vp, route, routeName) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const state = await page.locator("[data-community-tab]").evaluateAll(
      (els, clicked) => ({
        activeCount: els.filter((el) => el.classList.contains("is-active"))
          .length,
        clickedActive: els[clicked]?.classList.contains("is-active") || false,
        activeKey:
          els
            .find((el) => el.classList.contains("is-active"))
            ?.getAttribute("data-community-tab") || null,
      }),
      i,
    );
    await screenshot(
      ...
      vp,
      routeName,
      `community-tab-${String(i + 1).padStart(2, "0")}`,
    );
...
async function exerciseAnchorNav(page, vp, route, routeName) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (!state.targetExists || state.hash !== href)
      finding(vp, route, "anchor-nav", { href, ...state });
  }
}
...
for (const vp of viewports) {
  ...
  page.on("pageerror", (err) =>
    finding(vp.name, currentRoute, "page-error", String(err)),
  );
  page.on("console", (msg) => {
    if (msg.type() === "error")
      finding(vp.name, currentRoute, "console-error", msg.text());
  });
  ...
  for (const route of routes) {
    ...
    if ((await navToggle.count()) && (await navToggle.isVisible())) {
      ...
      if (
        ...
        !["auto", "scroll"].includes(nav.overflowY)
      )
        finding(vp.name, route, "nav-clipped", nav);
      await page.keyboard.press("Escape");
      await page.waitForTimeout(450);
      if ((await navToggle.getAttribute("aria-expanded")) !== "false")
        finding(vp.name, route, "nav-close", "Escape did not close menu");
    }
    ...
    for (let i = 0, n = Math.min(await faqs.count(), 6); i < n; i++) {
      ...
      if ((await q.getAttribute("aria-expanded")) !== "true")
        finding(vp.name, route, "faq-open", `FAQ ${i + 1} failed`);
      await screenshot(
        page,
        vp.name,
        routeName,
    ...
    if (route === "/help/") {
      ...
      if ((await input.count()) && (await firstCard.count())) {
        const query = await firstCard.evaluate((el) => {
          const title =
            el.querySelector("h3,h4")?.textContent || el.textContent || "";
          return title.trim().slice(0, Math.min(5, title.trim().length));
        });
        await input.fill(query);
        await page.waitForTimeout(220);
        await screenshot(page, vp.name, routeName, "help-search");
        if ((await page.locator("[data-help-card]:visible").count()) === 0)
          finding(vp.name, route, "help-search", { query });
        await input.fill("");
        await page.waitForTimeout(120);
      }
    }
    ...
    if (route === "/contact/") {
      ...
      if ((await form.count()) && (await submit.count())) {
        ...
        const invalid = await form
          ...
          .evaluateAll((els) => els.filter((el) => !el.checkValidity()).length);
        if (invalid === 0)
          finding(
            vp.name,
            route,
            "contact-validation",
            "Empty required fields were considered valid",
          );
      }
    }
    ...
    for (let i = 0; i < scrollStops.length; i++) {
      ...
      await screenshot(
        ...
        vp.name,
        routeName,
        `scroll-${String(i + 1).padStart(2, "0")}`,
      );
    }
    ...
    if ((await topBtn.count()) && (await topBtn.isVisible())) {
      ...
      try {
        ...
        finding(vp.name, route, "scroll-top", {
...
if (report.findings.some((item) => severeKinds.has(item.kind)))
  process.exitCode = 1;
