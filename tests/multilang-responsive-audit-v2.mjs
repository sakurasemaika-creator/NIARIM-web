...
const outDir = process.env.AUDIT_OUT_DIR || "artifacts/multilang-responsive-audit-v2";
    ...
    for (const language of languages) {
      ...
      const state = await page.evaluate(() => {
        ...
        const textOverflows = [...document.querySelectorAll("main h1, main h2, main h3, main p, main li, main .btn, main label")]
          ...
          .slice(0, 20)
          .map((el) => ({ tag: el.tagName, cls: el.className, text: (el.textContent || "").trim().slice(0, 100), clientWidth: el.clientWidth, scrollWidth: el.scrollWidth }));

        const buttons = [...document.querySelectorAll(".btn-primary, .btn-accent")].map((el) => ({
        ...
        const strips = [...document.querySelectorAll(".fd-frame-strip-scroll")].map((strip) => {
          const sr = rect(strip);
          const current = strip.querySelector(".fd-frame-thumb.is-selected, .fd-frame-thumb.is-current, .fd-frame.is-active, .fd-frame.is-current");
          ...
          return {
            ...
            currentBorder: current ? parseFloat(getComputedStyle(current).borderTopWidth) || 0 : 0,
            pseudoVisible: pseudo.content && pseudo.content !== "none" && pseudo.display !== "none",
          };
        });
      ...
      if (state.lang !== language) add(id, "language-not-applied", { actual: state.lang });
      if (state.scrollWidth > state.clientWidth + 2) add(id, "page-horizontal-overflow", { clientWidth: state.clientWidth, scrollWidth: state.scrollWidth });
      if (state.textOverflows.length) add(id, "translated-text-overflow", state.textOverflows);

      for (const button of state.buttons) {
        if (button.background !== state.accent) add(id, "button-not-active-theme-accent", { expected: state.accent, ...button });
        if (button.rect && (button.rect.left < -1 || button.rect.right > state.clientWidth + 1)) add(id, "button-clipped", button);
      }

      if (route === "/" && state.hero) {
        const target = 320 / 569;
        const ratio = state.hero.width / state.hero.height;
        if (Math.abs(ratio - target) > 0.018) add(id, "hero-ratio", { expected: target, actual: ratio, hero: state.hero });
        if (state.hero.left < -1 || state.hero.right > state.clientWidth + 1) add(id, "hero-clipped", state.hero);
      }

      for (const strip of state.strips) {
        if (strip.current && Math.abs(strip.current.width - 50) > 1.5) add(id, "current-frame-width", strip);
        if (strip.current && Math.abs(strip.current.height - 50) > 1.5) add(id, "current-frame-height", strip);
        if (strip.current && Math.abs(strip.currentDelta) > 2) add(id, "current-frame-not-centered", strip);
        if (strip.cursor && Math.abs(strip.cursorDelta) > 2) add(id, "cursor-not-centered", strip);
        if (strip.currentBorder > 1.1) add(id, "duplicate-current-outline", strip);
        if (strip.pseudoVisible) add(id, "legacy-pseudo-current-marker-visible", strip);
      }

      for (const slider of state.sliders) {
        if (slider.height && Math.abs(slider.height - 4) > 1) add(id, "slider-height", slider);
      ...
      if (route === "/") {
        ...
        if (await scroller.count()) {
          ...
          if (end && (end.lastLeft < end.scrollerLeft - 2 || end.lastRight > end.scrollerRight + 2)) add(id, "app-preview-last-card-clipped", end);
        }
      }
    }
  }
...
const report = { generatedAt: new Date().toISOString(), combinations: viewports.length * languages.length * routes.length, findings, measurements };
await fs.writeFile(path.join(outDir, "report.json"), JSON.stringify(report, null, 2));
console.log(JSON.stringify({ ok: findings.length === 0, combinations: report.combinations, findings: findings.length, byKind: findings.reduce((a, x) => ((a[x.kind] = (a[x.kind] || 0) + 1), a), {}) }, null, 2));
