...
const outDir = process.env.AUDIT_OUT_DIR || 'artifacts/autonomous-browser-audit/deep-sections';
const viewports = [
...
for (const vp of viewports) {
  ...
  let routeNow = '';
  page.on('pageerror', error => add(vp.name, routeNow, 'page-error', String(error)));
  page.on('console', msg => {
    if (msg.type() === 'error') add(vp.name, routeNow, 'console-error', msg.text());
  });
  ...
  for (const target of targets) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const state = await section.evaluate(el => {
        ...
        const substantive = [...el.querySelectorAll('h1,h2,h3,h4,p,a,button,img,svg,.feature-diagram,.fd-app-screen,.fd-route-screen')]
          .filter(node => {
            const s = getComputedStyle(node);
            const nr = node.getBoundingClientRect();
            return s.display !== 'none' && s.visibility !== 'hidden' && Number(s.opacity) > 0.05 && nr.width > 1 && nr.height > 1;
          }).length;
        const mocks = [...el.querySelectorAll('.fd-app-screen,.fd-route-screen')].map(node => {
          ...
          return {
            ...
            visibleReveal: !node.closest('.reveal') || node.closest('.reveal').classList.contains('is-visible')
          };
        });
        return {
      ...
      if (state.width < 1 || state.height < 1 || state.display === 'none' || state.visibility === 'hidden' || state.opacity < 0.05 || state.substantive === 0) {
        add(vp.name, target.route, 'empty-section', { id, ...state });
      }
      for (const mock of state.mocks) {
        if (mock.opacity < 0.99 || !mock.visibleReveal || Math.abs(mock.ratio - 9 / 16) > 0.03) {
          add(vp.name, target.route, 'screen-mock', { id, ...mock });
        }
      }

      const safeId = id.replace(/[^a-zA-Z0-9_-]+/g, '-');
      const file = path.join(outDir, `${vp.name}__${target.name}__${String(i + 1).padStart(2, '0')}__${safeId}.png`);
...
console.log(JSON.stringify({ deepSectionFindings: report.findings.length, byKind: counts, screenshots: report.screenshots.length }, null, 2));
