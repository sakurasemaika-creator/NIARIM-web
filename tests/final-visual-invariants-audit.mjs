  ...
  for (const language of languages) {
    ...
    for (const route of routes) {
      ...
      const state = await page.evaluate(() => {
        ...
        const logicalScale = (el) => {
          const host = el.closest(
            ".fd-app-screen, .fd-route-screen, .hero-app-preview-source",
          );
          if (!host) return 1;
          const declaredFit = parseFloat(
            getComputedStyle(host).getPropertyValue("--fd-fit"),
          );
          if (
            Number.isFinite(declaredFit) &&
            declaredFit > 0 &&
            declaredFit !== 1
          )
            return declaredFit;
          ...
          return logicalWidth > 0 && hostRect.width > 0
            ? hostRect.width / logicalWidth
            : 1;
        };
        const buttons = [
        ...
        ].map((el) => ({ id: el.id, ...themeValues(el) }));
        return {
          accent,
          accentStrong,
          accentStrongDark,
          ...
          gallery: scroller
            ? { count: cards.length, cards: galleryCards }
            : null,
          featureThemes,
        };
      });
      ...
      if (route === "/" && state.gallery) {
        if (state.gallery.count !== expectedGalleryCards)
          failures.push({
            id,
        ...
        if (populated.length !== expectedGalleryCards)
          failures.push({
            id,
          ...
          if (
            accents.size !== expectedGalleryCards ||
            bezels.size !== expectedGalleryCards
          )
            failures.push({
              id,
  ...
  JSON.stringify(
    {
      ...
      checks: [
        ...
        "50x50 logical frames across scaled mocks",
        "current frame centered",
        "hero 320:569 ratio",
        "all seven app-preview ratios",
        "seven distinct app-preview themes",
