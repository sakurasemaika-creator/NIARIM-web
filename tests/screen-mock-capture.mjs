...
const context = await browser.newContext({ viewport: { width: 390, height: 844 }, deviceScaleFactor: 1, locale: 'ja-JP' });
...
for (const route of ['/', '/features/']) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const sectionName = (meta.sectionId || (meta.inGallery ? 'gallery' : 'screen')).replace(/[^a-zA-Z0-9_-]+/g, '-');
    const fileName = `${routeName}__${String(i + 1).padStart(2, '0')}__${sectionName}.png`;
    await mock.screenshot({ path: path.join(outDir, fileName), animations: 'disabled' });
