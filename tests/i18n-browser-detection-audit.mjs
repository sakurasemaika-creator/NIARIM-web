...
for (const test of cases) {
  ...
  if (actual !== test.expected) {
    findings.push({ kind: 'locale-detection', locale: test.locale, expected: test.expected, actual });
  }
  await context.close();
}
...
console.log(JSON.stringify({ findings: findings.length, cases: cases.length + 2, details: findings }, null, 2));
