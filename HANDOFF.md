...
見た目の確認手順（実際に配信して描画したものを撮り、数値と突き合わせて
確かめる進め方）は `AI-VERIFICATION.md` に切り出しました。AIモデルが
自律的に見た目を直すときは、まずそちらを読んでください。値としては正常
なのに見え方が違う不具合（フォントの字形が閲覧環境で変わる、実機に無い
帯が見える等）は、DOM検査だけでは拾えません。
