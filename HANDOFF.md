...
**初めて触る場合は `ONBOARDING.md` から読んでください。** 全体像・
最初の1歩・守るべき禁止事項と、目的別の文書の地図があります。

このファイルは「経緯」と「ハマりどころ」に特化しています。仕組みの説明は
次に切り出しました。

- `AI-VERIFICATION.md` … 見た目を自分で撮って確かめる手順
- `docs/CSS-LAYERS.md` … CSSの読み込み順と詳細度（事故が一番多い場所）
- `docs/SCREEN-MOCKS.md` … 画面再現図の仕組みと足し方
- `docs/I18N.md` … 7言語の文言の足し方
- `docs/FONTS.md` … フォントのサブセット化
- `docs/AUDITS.md` … 監査スクリプト23本が何を守っているか
...
- **アプリ本体リポジトリ**: `sakurasemaika-creator/NIARIM`（Flutter製、private）。
  ...
  - **サイト側のほうが正しい箇所がある**点に注意。アプリの `help*Desc` は
    実装より古いことがあり、2026/09/03時点では次の5つがそれに該当した。
    突き合わせのたびにアプリ側へ盲目的に上書きしないこと。
    - 演出／描画フィルターの名称：アプリのヘルプ本文は「ピクセレート」だが
      UIの `filterNamePixelate` は「ドット絵フィルター」。サイトはUI側に合わせている
    - 描画フィルターの一覧：実在する `filterNameLensDistortion`（眼鏡断層
      フィルター）がアプリのヘルプ本文に載っていない
    - トーン塗り：`toneImportFailedSnackbar` があり自作トーンの読み込みは
      実在するが、アプリのヘルプ本文に記載がない
    - 引き継ぎ：`transferProjectsHint` の通りプロジェクトも含められるが、
      アプリのヘルプ本文は「含められない」と読める書き方のまま
    - 動画書き出しの見出し：`exportFormatAvi` があり AVI 書き出しは実在するが、
      `helpVideoExportTitle` は「MP4・WebM・GIF」でAVIが抜けている
