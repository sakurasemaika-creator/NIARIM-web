...
(function () {
  ...
  var HELP = {
    ja: {
      ...
      "help.entry9.desc":
        "よく使うブラシ・ツールの組み合わせをあらかじめ登録しておき、ボタン一つで順番に切り替えられる機能です。キャンバス上の早替えツールボタンを長押しまたは上スワイプすると、登録・並べ替え・削除ができる管理ポップアップが開きます。ドラッグで並び順を変更できます。",
    ...
    en: {
      ...
      "help.entry9.desc":
        "Register combinations of brushes and tools you use often, and cycle through them with a single button tap. Long-press or swipe up on the Quick Tool button on the canvas to open a management popup where you can add, reorder, and remove entries. Drag to change their order.",
      ...
      "help.entry52.title": "Video Export (MP4, WebM, GIF, AVI)",
      "help.entry52.desc":
    ...
    "zh-Hans": {
      ...
      "help.entry9.desc":
        "预先登记常用的画笔・工具组合，只需点击一个按钮即可依次切换。长按或向上滑动画布上的快捷工具按钮，即可打开可以添加、排序、删除的管理弹窗。可通过拖动改变顺序。",
      ...
      "help.entry29.desc":
        "在填充工具的设置中把纯色填充切换为网点填充，就能用选中的网点、线条等网点图案进行填充。还提供像素模式专用的方格纹、格子纹网点，可以做出符合像素画质感的上色效果。荷叶边、针织纹理、连裤袜等手绘起来很费工夫的细致花纹也能轻松画出。还可以自制原创网点并添加使用，或与其他用户分享。",
      ...
      "help.entry51.desc":
        "将设置、素材、笔刷、自动上色设置、主题、调色板（取色器・像素画专用）等整个应用环境一次性转移到其他设备的功能。可以通过复选框单独选择要转移的项目，还可以选择性地一并包含制作中的项目（所选项目会连同素材、字体一起完整转移）。如果只想传递单个项目，也可以改用「共享（.niashare）」。",
      "help.entry52.title": "视频导出（MP4・WebM・GIF・AVI）",
      "help.entry52.desc":
      ...
      "help.entry53.desc":
        "导出视频、图片期间会显示进度，也可以中途取消。所需时间会因设备性能而异。",
    ...
    "zh-Hant": {
      ...
      "help.entry9.desc":
        "預先登記常用的畫筆・工具組合，只需點擊一個按鈕即可依序切換。長按或向上滑動畫布上的快速切換工具按鈕，即可開啟可以新增、排序、刪除的管理彈窗。可透過拖曳改變順序。",
      "help.entry10.title": "印章工具",
      "help.entry10.desc":
      ...
      "help.entry29.desc":
        "在填色工具的設定中把純色填滿切換為網點填滿，就能用選中的網點、線條等網點圖案進行填滿。還提供像素模式專用的方格紋、格子紋網點，可以做出符合像素畫質感的上色效果。荷葉邊、針織紋理、褲襪等手繪起來很費工夫的細緻花紋也能輕鬆畫出。還可以自製原創網點並新增使用，或與其他使用者分享。",
      "help.entry30.title": "像素模式",
      "help.entry30.desc":
      ...
      "help.entry40.desc":
        "可套用於場景或影格的畫面效果（模糊・色調校正・光暈・像素畫等）。在不改變手繪畫面本身的前提下，作為演出手段調整整個畫面觀感時使用。像素畫也可以選擇配色方式（不指定顏色・指定顏色・指定顏色數・從調色盤選擇）。 演出濾鏡可以疊加套用多個，其套用順序依照時間軸上的排列順序。拖曳濾鏡清單重新排序後，實際反映到畫面上的順序也會隨之改變。 一種演出濾鏡，會逐影格改變類似膠片顆粒感的雜訊。可以用滑桿調整強度、數量（雜訊出現的密度）和顆粒大小。回到同一影格時雜訊形態相同（拖曳時不會閃爍），播放時顆粒則會呈現出跳動的效果。 用來表現畫面中降雨的演出濾鏡。可以用滑桿調整降雨強度（雨滴數量）、速度、雨滴大小和風向角度。每滴雨都會隨著影格數推進以固定速度持續落下，呈現自然的降雨效果。",
      ...
      "help.entry51.desc":
        "將設定、素材、筆刷、自動上色設定、主題、調色盤（取色器・像素畫專用）等整個應用程式環境一次性轉移到其他裝置的功能。可以透過核取方塊個別選擇要轉移的項目，還可以選擇性地一併包含製作中的專案（所選專案會連同素材、字型一起完整轉移）。如果只想傳遞單一專案，也可以改用「共用（.niashare）」。",
      "help.entry52.title": "影片匯出（MP4・WebM・GIF・AVI）",
      "help.entry52.desc":
      ...
      "help.entry53.desc":
        "匯出影片、圖片期間會顯示進度，也可以中途取消。所需時間會依裝置效能而異。",
    ...
    ko: {
      ...
      "help.entry9.desc":
        "자주 쓰는 브러시・도구 조합을 미리 등록해두고 버튼 하나로 순서대로 전환할 수 있는 기능입니다. 캔버스의 빠른 도구 버튼을 길게 누르거나 위로 스와이프하면 등록・순서 변경・삭제가 가능한 관리 팝업이 열립니다. 드래그로 순서를 바꿀 수 있습니다.",
      ...
      "help.entry29.desc":
        "버킷 도구 설정에서 단색 채우기를 톤 채우기로 바꾸면 선택한 망점・선 무늬 등의 톤 패턴으로 채울 수 있습니다. 픽셀 모드 전용 체크무늬・격자무늬 톤도 준비되어 있어, 도트 그림 질감을 살린 채색이 가능합니다. 프릴이나 니트, 타이츠 등 손그림으로는 손이 많이 가는 세밀한 무늬도 간단히 그릴 수 있습니다. 오리지널 톤을 직접 만들어 추가하거나 다른 사용자와 공유할 수도 있습니다.",
      ...
      "help.entry40.desc":
        "씬이나 프레임에 적용할 수 있는 영상 효과（블러・색조 보정・글로우・픽셀 아트 등）입니다. 손그림 자체는 바꾸지 않고, 연출로서 화면 전체의 느낌을 조정하고 싶을 때 사용합니다. 픽셀 아트는 배색 방식（색을 지정하지 않음・색을 지정함・색 수를 지정함・팔레트에서 선택）도 고를 수 있습니다. 연출 필터는 여러 개를 겹쳐서 적용할 수 있으며, 그 적용 순서는 타임라인상의 배열 순서를 따릅니다. 필터 목록을 드래그로 재배열하면 실제로 화면에 반영되는 순서도 바뀝니다. 필름의 입자감 같은 노이즈를 프레임마다 변화시키며 적용하는 연출 필터입니다. 강도・양（노이즈가 올라가는 밀도）・입자 크기를 슬라이더로 조정할 수 있습니다. 같은 프레임으로 돌아오면 같은 입자 모양이 되므로 스크러빙 중에는 깜빡이지 않고, 재생하면 입자가 움직이는 것처럼 보입니다. 화면에 내리는 비를 표현하는 연출 필터입니다. 내리는 정도（빗방울 수）・속도・빗방울 크기・바람 방향 각도를 슬라이더로 조정할 수 있습니다. 각 빗방울은 프레임이 진행될수록 일정한 속도로 계속 내리므로 자연스러운 비의 움직임이 됩니다.",
      ...
      "help.entry51.desc":
        "설정・소재・브러시・자동 채색 설정・테마・팔레트(컬러 피커・도트 그림 전용) 등 앱 전체 환경을 다른 기기로 한꺼번에 이전하는 기능입니다. 이전할 항목은 체크박스로 개별 선택할 수 있으며, 제작 중인 프로젝트도 선택해서 포함할 수 있습니다(선택한 프로젝트는 소재・글꼴까지 통째로 이전됩니다). 개별 프로젝트만 전달하고 싶을 때는 「공유（.niashare）」도 사용할 수 있습니다.",
      "help.entry52.title": "동영상 내보내기（MP4・WebM・GIF・AVI）",
      "help.entry52.desc":
      ...
      "help.entry69.title": "레이어 다중 선택・일괄 조작",
      "help.entry69.desc":
    ...
    fr: {
      ...
      "help.entry9.desc":
        "Enregistrez des combinaisons de pinceaux et d'outils que vous utilisez souvent, et faites défiler entre elles d'une simple pression sur un bouton. Faites un appui long ou balayez vers le haut sur le bouton Outil rapide du canevas pour ouvrir une fenêtre de gestion où ajouter, réorganiser et supprimer des entrées. Faites glisser pour changer leur ordre.",
      ...
      "help.entry52.title": "Export vidéo (MP4, WebM, GIF, AVI)",
      "help.entry52.desc":
    ...
    es: {
      ...
      "help.entry9.desc":
        "Registra combinaciones de pinceles y herramientas que uses a menudo, y alterna entre ellas con un solo toque de botón. Mantén pulsado o desliza hacia arriba el botón Herramienta rápida del lienzo para abrir un panel de gestión donde añadir, reordenar y eliminar entradas. Arrastra para cambiar su orden.",
      ...
      "help.entry52.title": "Exportación de vídeo (MP4, WebM, GIF, AVI)",
      "help.entry52.desc":
      ...
      "help.entry68.title": "Pinceles personalizados",
      "help.entry68.desc":
      ...
      "help.entry69.title": "Selección múltiple y acciones masivas de capas",
      "help.entry69.desc":
