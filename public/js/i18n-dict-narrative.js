...
(function () {
  ...
  "zh-Hans": {
    ...
    "narrative.advanced.9.body": "可以为每个图层应用滤镜。模糊、锐化、USM锐化、色调曲线、色阶、暗角、噪点、复古动画、显像管、动画风、描边、马赛克、鱼眼镜头等等。此外，眼镜断层滤镜还能再现透过不同度数镜片观察景物时产生的畸变效果。",
    "narrative.advanced.10.heading": "演出滤镜",
    "narrative.advanced.10.body": "这是一种可按帧或按场景应用的影像效果。由于可以一次性处理多个帧，因此能为整部作品或某个场景统一添加相同的演出效果。叠加噪点、下雨等演出效果，来完成整部作品的氛围。想直接加工图层，就用描绘滤镜；想为整个帧或场景增添演出效果，就用演出滤镜。可根据用途灵活选用。",
  ...
  "zh-Hant": {
    ...
    "narrative.advanced.9.body": "可以為每個圖層套用濾鏡。模糊、銳化、USM銳化、色調曲線、色階、暈影、雜訊、復古動畫、映像管、動畫風、描邊、馬賽克、魚眼鏡頭等等。此外，眼鏡斷層濾鏡還能重現透過不同度數鏡片觀察景物時產生的扭曲效果。",
    "narrative.advanced.10.heading": "演出濾鏡",
    "narrative.advanced.10.body": "這是一種可依影格或依場景套用的影像效果。由於可以一次處理多個影格，因此能為整部作品或某個場景統一加上相同的演出效果。疊加雜訊、下雨等演出效果，來完成整部作品的氛圍。想直接加工圖層，就用描繪濾鏡；想為整個影格或場景增添演出效果，就用演出濾鏡。可依用途靈活選用。",
  ...
  "ko": {
    ...
    "narrative.advanced.9.body": "레이어별로 필터를 적용할 수 있습니다. 블러, 샤프, 언샤프 마스크, 톤 커브, 레벨 보정, 비네트, 노이즈, 레트로 애니메, 브라운관, 애니메풍, 테두리, 픽셀화, 어안 렌즈 등. 게다가 안경 단면 필터에서는 렌즈 도수에 따라 렌즈 너머의 풍경이 어떻게 왜곡되어 보이는지를 재현할 수 있습니다.",
    "narrative.advanced.10.heading": "연출 필터",
    "narrative.advanced.10.body": "이쪽은 프레임이나 씬 단위로 적용할 수 있는 영상 효과입니다. 여러 프레임을 한 번에 처리할 수 있어, 작품 전체나 하나의 씬에 같은 연출을 더할 수도 있습니다. 노이즈나 비 등의 연출을 겹쳐 작품 전체의 분위기를 완성할 수 있습니다. 레이어를 직접 가공하려면 드로잉 필터. 프레임이나 씬 전체를 연출하려면 연출 필터. 용도에 맞춰 구분해 사용할 수 있습니다.",
