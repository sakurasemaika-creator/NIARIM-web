...
(async () => {
  ...
  for (const path of PAGES) {
    for (const lang of LANGS) {
      ...
      await page.evaluate(l => { try { localStorage.setItem('niarim_lang', l); } catch (e) {} }, lang);
      ...
      const res = await page.evaluate(() => {
        ...
        while ((node = walker.nextNode())) {
          ...
          if (!/^\s*["']?Kuramubon/.test(getComputedStyle(el).fontFamily || '')) continue;
  ...
  process.stdout.write(JSON.stringify({ chars: [...chars].join(''), keys: [...keys] }));
})();
