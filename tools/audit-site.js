...
(async () => {
  const configuredExecutable =
    ...
    (process.platform === "linux" ? "/opt/pw-browsers/chromium" : null);
  ...
  for (const path of PAGES) {
    ...
    if (!resp || resp.status() >= 400)
      note(path, "HTTP", String(resp && resp.status()));
    ...
    const res = await page.evaluate(() => {
      ...
      out.noName = [...document.querySelectorAll("a[href], button")]
        ...
        .map(
          (el) => el.tagName + " " + (el.getAttribute("href") || el.className),
        );
      ...
      out.focusableInCollapsed = [
        ...document.querySelectorAll(
          ".faq-item:not(.is-open) .faq-answer a[href], .faq-item:not(.is-open) .faq-answer button",
        ),
      ].filter((el) => {
        el.focus();
        return document.activeElement === el;
      }).length;
      out.faqMissingAriaControls = [
      ...
      const levels = [...document.querySelectorAll("h1,h2,h3,h4,h5,h6")]
        .filter((h) => !isHidden(h))
        .map((h) => +h.tagName[1]);
      out.headingJumps = levels.filter(
        (l, i) => i > 0 && l > levels[i - 1] + 1,
      ).length;
      ...
      out.canonical =
        out.noindex || !!document.querySelector("link[rel=canonical]");
      out.title = (document.title || "").trim().length;
      out.desc =
        out.noindex || !!document.querySelector('meta[name="description"]');
      return out;
    });
    ...
    if (external.length)
      note(path, "外部リクエスト", [...new Set(external)].join(" | "));
    if (res.noName.length)
      note(path, "アクセシブル名なし", res.noName.join(", "));
    if (res.dead.length)
      note(path, "行き先の無い表示リンク", res.dead.join(", "));
    if (res.unlabeled.length)
      note(path, "ラベルなし入力", res.unlabeled.join(", "));
    if (res.focusableInCollapsed)
      note(path, "畳んだFAQ内がタブ可能", res.focusableInCollapsed);
    if (res.faqMissingAriaControls)
      note(path, "aria-controls欠落", res.faqMissingAriaControls);
    ...
    for (const w of WIDTHS) {
      ...
      if (o.scrollW > o.clientW + 1)
        note(path, "横スクロール発生", `${w}px: ${o.scrollW} > ${o.clientW}`);
    }
    await page.close();
  }
